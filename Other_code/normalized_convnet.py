...
def fc_layer(input_variable, layer_shape, random_state):
    np_W = 0.2 * (random_state.rand(*layer_shape).astype(theano.config.floatX) - 0.5)
...
# filter 2 using filter out from 1
filter_shape = (64, filter_shape[0], 2, 2)
pool_shape = (2, 2)
stride = (1, 1)
out, l_params = conv_layer(out, filter_shape, pool_shape, stride, random_state)
params += l_params

#filter 3 using filter out from 2
