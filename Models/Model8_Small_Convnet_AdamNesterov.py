...
def main(save_to, num_epochs, feature_maps=None, mlp_hiddens=None,
    ...
    if feature_maps is None:
        feature_maps = [32, 64, 64, 128, 128]
    if mlp_hiddens is None:
        mlp_hiddens = [1000]
    if conv_sizes is None:
        conv_sizes = [7, 5, 5, 3, 3]
    ...
    extensions.append(Checkpoint("Model8_AdamNesterov.pkl", after_epoch=True, after_training=True, save_separately=['log']))
    extensions.append(ProgressBar())
    extensions.append(Printing())

    host_plot='http://hades:5090'
    extensions.append(Plot('%s %s @ %s' % ('CNN8_AdamNesterov', datetime.datetime.now(), socket.gethostname()),
